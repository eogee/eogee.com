...
class Model{
    ...
    public static function getTableComment($table = null)
    {
        ...
        // 获取表注释
        $result = Database::tableComment($table);
    ...
    public static function getTableFieldComment($table = null)
    {
        ...
        // 获取字段注释
        $colComments = Database::columnComment($table); // 获取数据表中的字段注释
    ...
    public static function columnIsnullable($table = null)
    {
        ...
        // 获取字段是否为空的信息
        $columnNullableInfo = Database::columnIsnullable($table); // 获取数据表中字段是否为空
    ...
    public static function listApi($table = null, $field = null, $page = 1, $limit = 10)
    {
        ...
        // 确定查询的字段
        if (empty($field)) {
            $field = Database::hasCol('name', $table) ? 'name' : 'id';
        }
        ...
        if (isset($_GET['parentId'])) {
            ...
            // 生成查询条件
            $where = Database::hasDeletedAt($table)
                ? "WHERE deleted_at IS NULL AND parentId = $parentId"
                : "WHERE parentId = $parentId";
    
            // 查询数据和计数
            $data['data'] = Database::select($table, $where, "LIMIT $offset, $limit");
            ...
            // 检查是否有 deleted_at 字段
            $where = Database::hasDeletedAt($table) ? "WHERE deleted_at IS NULL" : "";
    ...
    public static function recycleApi($table = null, $field = null, $page = 1, $limit = 10)
    {
        ...
        $field = $field ?: (Database::hasCol('name', $table) ? 'name' : 'id');
    ...
    public static function search($table, $page, $limit, $search, $field)
    {
        ...
        // 执行查询以获取数据
        $data['data'] = Database::select($table, $likeClause, "LIMIT $offset, $limit");
        $dataCount = count(Database::select($table, $likeClause)); // 获取总数
    ...
    public static function searchOption($field)
    {
        ...
        $fieldComments = self::getTableFieldComment(Url::getTable()); // 获取字段注释
    ...
    public static function insert()
    {
        ...
        // 执行插入操作
        $result = Database::insert(Url::getTable(), $_POST);
    ...
    public static function edit($table = null)
    {
        ...
        // 执行更新操作
        $result = Database::update($table, $_POST, "WHERE id = $id");
    ...
    public static function show($table = null, $id = null)
    {
        ...
        if (empty($id)) { // 如果没有 ID，则获取第一条数据
            $result = Database::select($table, "", "LIMIT 1");
            if (!empty($result)) {
            ...
            $result = Database::select($table, "WHERE id = " . intval($id));
            if (!empty($result)) {
                $data['data'] = $result[0]; // 存在则获取数据
            } else {
                $data['data'] = []; // 没有数据则设为空
    ...
    public static function showAll($table = null, $id = null, $sort = null, $where = null, $childTable = null, $parentKey = null)
    {
        ...
        if (!empty($sort)) {
            $data = Database::selectOrder($table, $sort, $whereClause);
        } else {
            $data = Database::select($table, $whereClause);
        }
        ...
        if (!empty($childTable)) {
            ...
            foreach ($data as $key => $value) {
                $childConditions = Database::hasDeletedAt($childTable) 
                    ? "WHERE $parentKey = " . $value['id'] . " AND deleted_at IS NULL" 
                    : "WHERE $parentKey = " . $value['id'];
                $data[$key]['childData'] = Database::selectOrder($childTable, $sort, $childConditions);
            }
        }
        return $data; // 返回结果
    }
    ...
    public static function showAllWithChild($table = null, $id = null, $childTable = null, $parentKey = null)
    {
        ...
        if (empty($id)) {
            // 获取数据
            if (Database::hasDeletedAt($table)) {
                $data['data'] = Database::select($table, "WHERE deleted_at IS NULL");
            } else {
                $data['data'] = Database::select($table);
            }
        }else{
            // 获取数据
            if (Database::hasDeletedAt($table)) {
        ...
        if (!empty($childTable) && $parentIdValue !== null) {
            $data['childData'] = Database::selectOrder($childTable, "sort", "WHERE $parentKey = $parentIdValue");
        } else {
    ...
    public static function deleteSoft()
    {
        ...
        // 执行更新操作
        $result = Database::update(Url::getTable(), $data, "WHERE id = $id");
    ...
    public static function deleteSoftBatch()
    {
        ...
        return Database::update(Url::getTable(), $data, "WHERE id IN ($idsString)");
    }
    /**
     * Summary of delete
     * 彻底删除
    ...
    public static function delete()
    {
        ...
        // 执行删除操作
        $result = Database::delete(Url::getTable(), "WHERE id = $id");
    ...
    public static function deleteBatch()
    {
        ...
        // 使用参数化查询更新数据
        return Database::delete(Url::getTable(), "WHERE id IN ($idsString)");
    }
    /**
     * Summary of restore
     * 还原数据
    ...
    public static function restore()
    {
        ...
        $result = Database::deleteDate(Url::getTable(), "WHERE id = $id");
    ...
    public static function restoreBatch()
    {
        ...
        // 执行批量还原操作
        $result = Database::deleteDate(Url::getTable(), "WHERE id IN ($idsString)");
